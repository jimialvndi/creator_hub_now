...
<section class="py-20 bg-white">
    <div class="container mx-auto px-4">
        <div class="text-center mb-12">
            <h2 class="text-4xl font-bold text-primary mb-4">Kelas Online Terbaru</h2>
            <p class="text-gray-600 text-lg">Tingkatkan skill Anda dengan materi eksklusif</p>
        </div>

        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-12">
            @forelse($featuredCourses as $course)
            <div class="bg-gray-50 rounded-2xl overflow-hidden border border-gray-100 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-2 group flex flex-col">
                <div class="relative overflow-hidden h-48 bg-gray-200">
                    <img src="{{ $course->thumbnail ? asset('storage/' . $course->thumbnail) : 'https://via.placeholder.com/600x400?text=Course+Preview' }}" 
                         class="w-full h-full object-cover group-hover:scale-105 transition duration-300" 
                         alt="{{ $course->title }}">
                    <div class="absolute top-4 left-4">
                        <span class="bg-white/90 backdrop-blur px-3 py-1 rounded-full text-xs font-bold shadow-sm uppercase text-primary">
                            {{ $course->access_level }}
                        </span>
                    </div>
                </div>
                ...
                <div class="p-6 flex-1 flex flex-col">
                    ...
                    <a href="{{ route('courses.show', $course->slug) }}" class="block text-center bg-primary text-white py-3 rounded-lg font-semibold hover:bg-blue-700 transition group-hover:bg-accent group-hover:text-primary">
                        Pelajari Sekarang
                    </a>
                </div>
            </div>
            ...
            <div class="col-span-3 text-center py-10">
                <p class="text-gray-500 italic">Belum ada kelas yang dipublikasikan.</p>
            </div>
            @endforelse
        </div>

        <div class="text-center">
            <a href="{{ route('courses.index') }}" class="inline-block bg-primary text-white px-8 py-4 rounded-full font-bold text-lg hover:bg-blue-700 transition">
                Lihat Semua Kelas
            </a>
        </div>
    </div>
